...
class TestConjugator:
    ...
    def test_init(self, conjugator):
        ...
        assert conjugator._present_personal_suffixes == ["م", "ی", "د", "یم", "ید", "ند"]
    ...
    def test_simple_past_passive(self, conjugator):
        ...
        expected = ['شناخته شدم', 'شناخته شدی', 'شناخته شد', 'شناخته شدیم', 'شناخته شدید', 'شناخته شدند']
        assert result == expected
        
        # Test with another verb
        result = conjugator.simple_past("دید", passive=True)
        expected = ['دیده شدم', 'دیده شدی', 'دیده شد', 'دیده شدیم', 'دیده شدید', 'دیده شدند']
        assert result == expected

    def test_simple_past_negative_passive(self, conjugator):
        # Test with example from docstring
        result = conjugator.simple_past("شناخت", negative=True, passive=True)
        expected = ['شناخته نشدم', 'شناخته نشدی', 'شناخته نشد', 'شناخته نشدیم', 'شناخته نشدید', 'شناخته نشدند']
        assert result == expected
        
        # Test with another verb
        result = conjugator.simple_past("خواند", negative=True, passive=True)
        expected = ['خوانده نشدم', 'خوانده نشدی', 'خوانده نشد', 'خوانده نشدیم', 'خوانده نشدید', 'خوانده نشدند']
        assert result == expected
    ...
    def test_present_perfect_regular(self, conjugator):
        ...
        expected = ['شناخته‌ام', 'شناخته‌ای', 'شناخته است', 'شناخته‌ایم', 'شناخته‌اید', 'شناخته‌اند']
        assert result == expected
        
        # Test with another verb
        result = conjugator.present_perfect("خورد")
        expected = ['خورده‌ام', 'خورده‌ای', 'خورده است', 'خورده‌ایم', 'خورده‌اید', 'خورده‌اند']
        assert result == expected

    def test_present_perfect_negative(self, conjugator):
        # Test with example from docstring
        result = conjugator.present_perfect("شناخت", negative=True)
        expected = ['نشناخته‌ام', 'نشناخته‌ای', 'نشناخته است', 'نشناخته‌ایم', 'نشناخته‌اید', 'نشناخته‌اند']
        assert result == expected
        
        # Test with another verb
        result = conjugator.present_perfect("رفت", negative=True)
        expected = ['نرفته‌ام', 'نرفته‌ای', 'نرفته است', 'نرفته‌ایم', 'نرفته‌اید', 'نرفته‌اند']
        assert result == expected

    def test_present_perfect_passive(self, conjugator):
        # Test with example from docstring
        result = conjugator.present_perfect("شناخت", passive=True)
        expected = ['شناخته شده‌ام', 'شناخته شده‌ای', 'شناخته شده است', 'شناخته شده‌ایم', 'شناخته شده‌اید', 'شناخته شده‌اند']
        assert result == expected
        
        # Test with another verb
        result = conjugator.present_perfect("دید", passive=True)
        expected = ['دیده شده‌ام', 'دیده شده‌ای', 'دیده شده است', 'دیده شده‌ایم', 'دیده شده‌اید', 'دیده شده‌اند']
        assert result == expected
    ...
    def test_present_perfect_negative_passive(self, conjugator):
        ...
        expected = ['شناخته نشده‌ام', 'شناخته نشده‌ای', 'شناخته نشده است', 'شناخته نشده‌ایم', 'شناخته نشده‌اید', 'شناخته نشده‌اند']
        assert result == expected
        
        # Test with another verb
        result = conjugator.present_perfect("خواند", negative=True, passive=True)
        expected = ['خوانده نشده‌ام', 'خوانده نشده‌ای', 'خوانده نشده است', 'خوانده نشده‌ایم', 'خوانده نشده‌اید', 'خوانده نشده‌اند']
        assert result == expected
    ...
    def test_past_continuous_regular(self, conjugator):
        ...
        expected = ['می‌شناختم', 'می‌شناختی', 'می‌شناخت', 'می‌شناختیم', 'می‌شناختید', 'می‌شناختند']
        assert result == expected
    ...
    def test_past_continuous_negative(self, conjugator):
        ...
        expected = ['نمی‌شناختم', 'نمی‌شناختی', 'نمی‌شناخت', 'نمی‌شناختیم', 'نمی‌شناختید', 'نمی‌شناختند']
        assert result == expected
    ...
    def test_past_continuous_passive(self, conjugator):
        ...
        expected = ['شناخته می‌شدم', 'شناخته می‌شدی', 'شناخته می‌شد', 'شناخته می‌شدیم', 'شناخته می‌شدید', 'شناخته می‌شدند']
        assert result == expected
        
        # Test with another verb
        result = conjugator.past_continuous("دید", passive=True)
        expected = ['دیده می‌شدم', 'دیده می‌شدی', 'دیده می‌شد', 'دیده می‌شدیم', 'دیده می‌شدید', 'دیده می‌شدند']
        assert result == expected
    ...
    def test_past_continuous_negative_passive(self, conjugator):
        ...
        expected = ['شناخته نمی‌شدم', 'شناخته نمی‌شدی', 'شناخته نمی‌شد', 'شناخته نمی‌شدیم', 'شناخته نمی‌شدید', 'شناخته نمی‌شدند']
        assert result == expected
        
        # Test with another verb
        result = conjugator.past_continuous("خواند", negative=True, passive=True)
        expected = ['خوانده نمی‌شدم', 'خوانده نمی‌شدی', 'خوانده نمی‌شد', 'خوانده نمی‌شدیم', 'خوانده نمی‌شدید', 'خوانده نمی‌شدند']
        assert result == expected
    ...
    def test_present_perfect_continuous_regular(self, conjugator):
        ...
        expected = ["می‌شناخته‌ام", "می‌شناخته‌ای", "می‌شناخته است", "می‌شناخته‌ایم", "می‌شناخته‌اید", "می‌شناخته‌اند"]
        assert result == expected
        
        # Test with another verb
        result = conjugator.present_perfect_continuous("خورد")
        expected = ["می‌خورده‌ام", "می‌خورده‌ای", "می‌خورده است", "می‌خورده‌ایم", "می‌خورده‌اید", "می‌خورده‌اند"]
        assert result == expected
    ...
    def test_present_perfect_continuous_negative(self, conjugator):
        ...
        expected = ["نمی‌شناخته‌ام", "نمی‌شناخته‌ای", "نمی‌شناخته است", "نمی‌شناخته‌ایم", "نمی‌شناخته‌اید", "نمی‌شناخته‌اند"]
        assert result == expected
        
        # Test with another verb
        result = conjugator.present_perfect_continuous("رفت", negative=True)
        expected = ["نمی‌رفته‌ام", "نمی‌رفته‌ای", "نمی‌رفته است", "نمی‌رفته‌ایم", "نمی‌رفته‌اید", "نمی‌رفته‌اند"]
        assert result == expected
    ...
    def test_present_perfect_continuous_passive(self, conjugator):
        ...
        expected = ["شناخته می‌شده‌ام", "شناخته می‌شده‌ای", "شناخته می‌شده است", "شناخته می‌شده‌ایم", "شناخته می‌شده‌اید", "شناخته می‌شده‌اند"]
        assert result == expected
        
        # Test with another verb
        result = conjugator.present_perfect_continuous("دید", passive=True)
        expected = ["دیده می‌شده‌ام", "دیده می‌شده‌ای", "دیده می‌شده است", "دیده می‌شده‌ایم", "دیده می‌شده‌اید", "دیده می‌شده‌اند"]
        assert result == expected

    def test_present_perfect_continuous_negative_passive(self, conjugator):
        # Test with example from docstring
        result = conjugator.present_perfect_continuous("شناخت", negative=True, passive=True)
        expected = ["شناخته نمی‌شده‌ام", "شناخته نمی‌شده‌ای", "شناخته نمی‌شده است", "شناخته نمی‌شده‌ایم", "شناخته نمی‌شده‌اید", "شناخته نمی‌شده‌اند"]
        assert result == expected
        
        # Test with another verb
        result = conjugator.present_perfect_continuous("خواند", negative=True, passive=True)
        expected = ["خوانده نمی‌شده‌ام", "خوانده نمی‌شده‌ای", "خوانده نمی‌شده است", "خوانده نمی‌شده‌ایم", "خوانده نمی‌شده‌اید", "خوانده نمی‌شده‌اند"]
        assert result == expected
    ...
    def test_past_perfect_regular(self, conjugator):
        ...
        expected = ['شناخته بودم', 'شناخته بودی', 'شناخته بود', 'شناخته بودیم', 'شناخته بودید', 'شناخته بودند']
        assert result == expected
        
        # Test with another verb
        result = conjugator.past_perfect("خورد")
        expected = ['خورده بودم', 'خورده بودی', 'خورده بود', 'خورده بودیم', 'خورده بودید', 'خورده بودند']
        assert result == expected

    def test_past_perfect_negative(self, conjugator):
        # Test with example from docstring
        result = conjugator.past_perfect("شناخت", negative=True)
        expected = ['نشناخته بودم', 'نشناخته بودی', 'نشناخته بود', 'نشناخته بودیم', 'نشناخته بودید', 'نشناخته بودند']
        assert result == expected
        
        # Test with another verb
        result = conjugator.past_perfect("رفت", negative=True)
        expected = ['نرفته بودم', 'نرفته بودی', 'نرفته بود', 'نرفته بودیم', 'نرفته بودید', 'نرفته بودند']
        assert result == expected

    def test_past_perfect_passive(self, conjugator):
        # Test with example from docstring
        result = conjugator.past_perfect("شناخت", passive=True)
        expected = ['شناخته شده بودم', 'شناخته شده بودی', 'شناخته شده بود', 'شناخته شده بودیم', 'شناخته شده بودید', 'شناخته شده بودند']
        assert result == expected
        
        # Test with another verb
        result = conjugator.past_perfect("دید", passive=True)
        expected = ['دیده شده بودم', 'دیده شده بودی', 'دیده شده بود', 'دیده شده بودیم', 'دیده شده بودید', 'دیده شده بودند']
        assert result == expected

    def test_past_perfect_negative_passive(self, conjugator):
        # Test with example from docstring
        result = conjugator.past_perfect("شناخت", negative=True, passive=True)
        expected = ['شناخته نشده بودم', 'شناخته نشده بودی', 'شناخته نشده بود', 'شناخته نشده بودیم', 'شناخته نشده بودید', 'شناخته نشده بودند']
        assert result == expected
        
        # Test with another verb
        result = conjugator.past_perfect("خواند", negative=True, passive=True)
        expected = ['خوانده نشده بودم', 'خوانده نشده بودی', 'خوانده نشده بود', 'خوانده نشده بودیم', 'خوانده نشده بودید', 'خوانده نشده بودند']
        assert result == expected
    ...
    def test_past_perfect_of_past_perfect_regular(self, conjugator):
        ...
        expected = ['شناخته بوده‌ام', 'شناخته بوده‌ای', 'شناخته بوده است', 'شناخته بوده‌ایم', 'شناخته بوده‌اید', 'شناخته بوده‌اند']
        assert result == expected
        
        # Test with another verb
        result = conjugator.past_perfect_of_past_perfect("خورد")
        expected = ['خورده بوده‌ام', 'خورده بوده‌ای', 'خورده بوده است', 'خورده بوده‌ایم', 'خورده بوده‌اید', 'خورده بوده‌اند']
        assert result == expected
    ...
    def test_past_perfect_of_past_perfect_negative(self, conjugator):
        ...
        expected = ['نشناخته بوده‌ام', 'نشناخته بوده‌ای', 'نشناخته بوده است', 'نشناخته بوده‌ایم', 'نشناخته بوده‌اید', 'نشناخته بوده‌اند']
        assert result == expected
        
        # Test with another verb
        result = conjugator.past_perfect_of_past_perfect("رفت", negative=True)
        expected = ['نرفته بوده‌ام', 'نرفته بوده‌ای', 'نرفته بوده است', 'نرفته بوده‌ایم', 'نرفته بوده‌اید', 'نرفته بوده‌اند']
        assert result == expected
    ...
    def test_past_perfect_of_past_perfect_passive(self, conjugator):
        ...
        expected = ['شناخته شده بوده‌ام', 'شناخته شده بوده‌ای', 'شناخته شده بوده است', 'شناخته شده بوده‌ایم', 'شناخته شده بوده‌اید', 'شناخته شده بوده‌اند']
        assert result == expected
        
        # Test with another verb
        result = conjugator.past_perfect_of_past_perfect("دید", passive=True)
        expected = ['دیده شده بوده‌ام', 'دیده شده بوده‌ای', 'دیده شده بوده است', 'دیده شده بوده‌ایم', 'دیده شده بوده‌اید', 'دیده شده بوده‌اند']
        assert result == expected

    def test_past_perfect_of_past_perfect_negative_passive(self, conjugator):
        # Test with example from docstring
        result = conjugator.past_perfect_of_past_perfect("شناخت", negative=True, passive=True)
        expected = ['شناخته نشده بوده‌ام', 'شناخته نشده بوده‌ای', 'شناخته نشده بوده است', 'شناخته نشده بوده‌ایم', 'شناخته نشده بوده‌اید', 'شناخته نشده بوده‌اند']
        assert result == expected
        
        # Test with another verb
        result = conjugator.past_perfect_of_past_perfect("خواند", negative=True, passive=True)
        expected = ['خوانده نشده بوده‌ام', 'خوانده نشده بوده‌ای', 'خوانده نشده بوده است', 'خوانده نشده بوده‌ایم', 'خوانده نشده بوده‌اید', 'خوانده نشده بوده‌اند']
        assert result == expected

    def test_past_perfect_of_past_perfect_empty_string(self, conjugator):
        # Test with empty string
        result = conjugator.past_perfect_of_past_perfect("")
        expected = ['ه بوده‌ام', 'ه بوده‌ای', 'ه بوده است', 'ه بوده‌ایم', 'ه بوده‌اید', 'ه بوده‌اند']
        assert result == expected

    def test_past_subjunctive_regular(self, conjugator):
        # Test with example from docstring
        result = conjugator.past_subjunctive("شناخت")
        expected = ['شناخته باشم', 'شناخته باشی', 'شناخته باشد', 'شناخته باشیم', 'شناخته باشید', 'شناخته باشند']
        assert result == expected
        
        # Test with another verb
        result = conjugator.past_subjunctive("خورد")
        expected = ['خورده باشم', 'خورده باشی', 'خورده باشد', 'خورده باشیم', 'خورده باشید', 'خورده باشند']
        assert result == expected

    def test_past_subjunctive_negative(self, conjugator):
        # Test with example from docstring
        result = conjugator.past_subjunctive("شناخت", negative=True)
        expected = ['نشناخته باشم', 'نشناخته باشی', 'نشناخته باشد', 'نشناخته باشیم', 'نشناخته باشید', 'نشناخته باشند']
        assert result == expected
        
        # Test with another verb
        result = conjugator.past_subjunctive("رفت", negative=True)
        expected = ['نرفته باشم', 'نرفته باشی', 'نرفته باشد', 'نرفته باشیم', 'نرفته باشید', 'نرفته باشند']
        assert result == expected

    def test_past_subjunctive_passive(self, conjugator):
        # Test with example from docstring
        result = conjugator.past_subjunctive("شناخت", passive=True)
        expected = ['شناخته شده باشم', 'شناخته شده باشی', 'شناخته شده باشد', 'شناخته شده باشیم', 'شناخته شده باشید', 'شناخته شده باشند']
        assert result == expected
        
        # Test with another verb
        result = conjugator.past_subjunctive("دید", passive=True)
        expected = ['دیده شده باشم', 'دیده شده باشی', 'دیده شده باشد', 'دیده شده باشیم', 'دیده شده باشید', 'دیده شده باشند']
        assert result == expected
    ...
    def test_past_subjunctive_negative_passive(self, conjugator):
        ...
        expected = ['شناخته نشده باشم', 'شناخته نشده باشی', 'شناخته نشده باشد', 'شناخته نشده باشیم', 'شناخته نشده باشید', 'شناخته نشده باشند']
        assert result == expected
        
        # Test with another verb
        result = conjugator.past_subjunctive("خواند", negative=True, passive=True)
        expected = ['خوانده نشده باشم', 'خوانده نشده باشی', 'خوانده نشده باشد', 'خوانده نشده باشیم', 'خوانده نشده باشید', 'خوانده نشده باشند']
        assert result == expected
    ...
    def test_past_progressive_regular(self, conjugator):
        ...
        expected = ['داشتم می‌شناختم', 'داشتی می‌شناختی', 'داشت می‌شناخت', 'داشتیم می‌شناختیم', 'داشتید می‌شناختید', 'داشتند می‌شناختند']
        assert result == expected
        
        # Test with another verb
        result = conjugator.past_progressive("خورد")
        expected = ['داشتم می‌خوردم', 'داشتی می‌خوردی', 'داشت می‌خورد', 'داشتیم می‌خوردیم', 'داشتید می‌خوردید', 'داشتند می‌خوردند']
        assert result == expected

    def test_past_progressive_passive(self, conjugator):
        # Test with example from docstring
        result = conjugator.past_progressive("شناخت", passive=True)
        expected = ['داشتم شناخته می‌شدم', 'داشتی شناخته می‌شدی', 'داشت شناخته می‌شد', 'داشتیم شناخته می‌شدیم', 'داشتید شناخته می‌شدید', 'داشتند شناخته می‌شدند']
        assert result == expected
        
        # Test with another verb
        result = conjugator.past_progressive("دید", passive=True)
        expected = ['داشتم دیده می‌شدم', 'داشتی دیده می‌شدی', 'داشت دیده می‌شد', 'داشتیم دیده می‌شدیم', 'داشتید دیده می‌شدید', 'داشتند دیده می‌شدند']
        assert result == expected

    def test_past_progressive_empty_string(self, conjugator):
        # Test with empty string
        result = conjugator.past_progressive("")
        expected = ['داشتم می‌م', 'داشتی می‌ی', 'داشت می‌', 'داشتیم می‌یم', 'داشتید می‌ید', 'داشتند می‌ند']
        assert result == expected
    ...
    def test_past_perfect_progressive_regular(self, conjugator):
        ...
        expected = ['داشته‌ام می‌شناخته‌ام', 'داشته‌ای می‌شناخته‌ای', 'داشته است می‌شناخته است', 
                    'داشته‌ایم می‌شناخته‌ایم', 'داشته‌اید می‌شناخته‌اید', 'داشته‌اند می‌شناخته‌اند']
        assert result == expected
        
        # Test with another verb
        result = conjugator.past_perfect_progressive("خورد")
        expected = ['داشته‌ام می‌خورده‌ام', 'داشته‌ای می‌خورده‌ای', 'داشته است می‌خورده است', 
                    'داشته‌ایم می‌خورده‌ایم', 'داشته‌اید می‌خورده‌اید', 'داشته‌اند می‌خورده‌اند']
        assert result == expected

    def test_past_perfect_progressive_passive(self, conjugator):
        # Test with example from docstring
        result = conjugator.past_perfect_progressive("شناخت", passive=True)
        expected = ['داشته‌ام شناخته می‌شده‌ام', 'داشته‌ای شناخته می‌شده‌ای', 'داشته است شناخته می‌شده است', 
                    'داشته‌ایم شناخته می‌شده‌ایم', 'داشته‌اید شناخته می‌شده‌اید', 'داشته‌اند شناخته می‌شده‌اند']
        assert result == expected
        
        # Test with another verb
        result = conjugator.past_perfect_progressive("دید", passive=True)
        expected = ['داشته‌ام دیده می‌شده‌ام', 'داشته‌ای دیده می‌شده‌ای', 'داشته است دیده می‌شده است', 
                    'داشته‌ایم دیده می‌شده‌ایم', 'داشته‌اید دیده می‌شده‌اید', 'داشته‌اند دیده می‌شده‌اند']
        assert result == expected

    def test_past_perfect_progressive_empty_string(self, conjugator):
        # Test with empty string
        result = conjugator.past_perfect_progressive("")
        expected = ['داشته‌ام می‌ه‌ام', 'داشته‌ای می‌ه‌ای', 'داشته است می‌ه است', 
                    'داشته‌ایم می‌ه‌ایم', 'داشته‌اید می‌ه‌اید', 'داشته‌اند می‌ه‌اند']
        assert result == expected
    ...
    def test_simple_present_passive(self, conjugator):
        ...
        expected = ['شناخته شوم', 'شناخته شوی', 'شناخته شود', 'شناخته شویم', 'شناخته شوید', 'شناخته شوند']
        assert result == expected
        
        # Test with another verb
        result = conjugator.simple_present("دید", "بین", passive=True)
        expected = ['دیده شوم', 'دیده شوی', 'دیده شود', 'دیده شویم', 'دیده شوید', 'دیده شوند']
        assert result == expected
    ...
    def test_simple_present_negative_passive(self, conjugator):
        ...
        expected = ['شناخته نشوم', 'شناخته نشوی', 'شناخته نشود', 'شناخته نشویم', 'شناخته نشوید', 'شناخته نشوند']
        assert result == expected
        
        # Test with another verb
        result = conjugator.simple_present("خواند", "خوان", negative=True, passive=True)
        expected = ['خوانده نشوم', 'خوانده نشوی', 'خوانده نشود', 'خوانده نشویم', 'خوانده نشوید', 'خوانده نشوند']
        assert result == expected
    ...
    def test_present_indicative_negative(self, conjugator):
        ...
        expected = ['نمی‌شناسم', 'نمی‌شناسی', 'نمی‌شناسد', 'نمی‌شناسیم', 'نمی‌شناسید', 'نمی‌شناسند']
        assert result == expected
    ...
    def test_present_indicative_passive(self, conjugator):
        ...
        expected = ['شناخته می‌شوم', 'شناخته می‌شوی', 'شناخته می‌شود', 'شناخته می‌شویم', 'شناخته می‌شوید', 'شناخته می‌شوند']
        assert result == expected
        
        # Test with another verb
        result = conjugator.present_indicative("دید", "بین", passive=True)
        expected = ['دیده می‌شوم', 'دیده می‌شوی', 'دیده می‌شود', 'دیده می‌شویم', 'دیده می‌شوید', 'دیده می‌شوند']
        assert result == expected

    def test_present_indicative_negative_passive(self, conjugator):
        # Test with example from docstring
        result = conjugator.present_indicative("شناخت", "شناس", negative=True, passive=True)
        expected = ['شناخته نمی‌شوم', 'شناخته نمی‌شوی', 'شناخته نمی‌شود', 'شناخته نمی‌شویم', 'شناخته نمی‌شوید', 'شناخته نمی‌شوند']
        assert result == expected
        
        # Test with another verb
        result = conjugator.present_indicative("خواند", "خوان", negative=True, passive=True)
        expected = ['خوانده نمی‌شوم', 'خوانده نمی‌شوی', 'خوانده نمی‌شود', 'خوانده نمی‌شویم', 'خوانده نمی‌شوید', 'خوانده نمی‌شوند']
        assert result == expected
    ...
    def test_present_subjunctive_passive(self, conjugator):
        ...
        expected = ['شناخته بشوم', 'شناخته بشوی', 'شناخته بشود', 'شناخته بشویم', 'شناخته بشوید', 'شناخته بشوند']
        assert result == expected
        
        # Test with another verb
        result = conjugator.present_subjunctive("دید", "بین", passive=True)
        expected = ['دیده بشوم', 'دیده بشوی', 'دیده بشود', 'دیده بشویم', 'دیده بشوید', 'دیده بشوند']
        assert result == expected

    def test_present_subjunctive_negative_passive(self, conjugator):
        # Test with example from docstring
        result = conjugator.present_subjunctive("شناخت", "شناس", negative=True, passive=True)
        expected = ['شناخته نشوم', 'شناخته نشوی', 'شناخته نشود', 'شناخته نشویم', 'شناخته نشوید', 'شناخته نشوند']
        assert result == expected
        
        # Test with another verb
        result = conjugator.present_subjunctive("خواند", "خوان", negative=True, passive=True)
        expected = ['خوانده نشوم', 'خوانده نشوی', 'خوانده نشود', 'خوانده نشویم', 'خوانده نشوید', 'خوانده نشوند']
        assert result == expected
    ...
    def test_present_progressive_regular(self, conjugator):
        ...
        expected = ['دارم می‌شناسم', 'داری می‌شناسی', 'دارد می‌شناسد', 'داریم می‌شناسیم', 'دارید می‌شناسید', 'دارند می‌شناسند']
        assert result == expected
        
        # Test with another verb
        result = conjugator.present_progressive("خورد", "خور")
        expected = ['دارم می‌خورم', 'داری می‌خوری', 'دارد می‌خورد', 'داریم می‌خوریم', 'دارید می‌خورید', 'دارند می‌خورند']
        assert result == expected

    def test_present_progressive_passive(self, conjugator):
        # Test with example from docstring
        result = conjugator.present_progressive("شناخت", "شناس", passive=True)
        expected = ['دارم شناخته می‌شوم', 'داری شناخته می‌شوی', 'دارد شناخته می‌شود', 'داریم شناخته می‌شویم', 'دارید شناخته می‌شوید', 'دارند شناخته می‌شوند']
        assert result == expected
        
        # Test with another verb
        result = conjugator.present_progressive("دید", "بین", passive=True)
        expected = ['دارم دیده می‌شوم', 'داری دیده می‌شوی', 'دارد دیده می‌شود', 'داریم دیده می‌شویم', 'دارید دیده می‌شوید', 'دارند دیده می‌شوند']
        assert result == expected

    def test_present_progressive_empty_string(self, conjugator):
        # Test with empty string
        result = conjugator.present_progressive("", "")
        expected = ['دارم می‌م', 'داری می‌ی', 'دارد می‌د', 'داریم می‌یم', 'دارید می‌ید', 'دارند می‌ند']
        assert result == expected
    ...
    def test_future_simple_regular(self, conjugator):
        ...
        expected = ['خواهم شناخت', 'خواهی شناخت', 'خواهد شناخت', 'خواهیم شناخت', 'خواهید شناخت', 'خواهند شناخت']
        assert result == expected
        
        # Test with another verb
        result = conjugator.future_simple("خورد")
        expected = ['خواهم خورد', 'خواهی خورد', 'خواهد خورد', 'خواهیم خورد', 'خواهید خورد', 'خواهند خورد']
        assert result == expected

    def test_future_simple_negative(self, conjugator):
        # Test with example from docstring
        result = conjugator.future_simple("شناخت", negative=True)
        expected = ['نخواهم شناخت', 'نخواهی شناخت', 'نخواهد شناخت', 'نخواهیم شناخت', 'نخواهید شناخت', 'نخواهند شناخت']
        assert result == expected
        
        # Test with another verb
        result = conjugator.future_simple("رفت", negative=True)
        expected = ['نخواهم رفت', 'نخواهی رفت', 'نخواهد رفت', 'نخواهیم رفت', 'نخواهید رفت', 'نخواهند رفت']
        assert result == expected

    def test_future_simple_passive(self, conjugator):
        # Test with example from docstring
        result = conjugator.future_simple("شناخت", passive=True)
        expected = ['شناخته خواهم شد', 'شناخته خواهی شد', 'شناخته خواهد شد', 'شناخته خواهیم شد', 'شناخته خواهید شد', 'شناخته خواهند شد']
        assert result == expected
        
        # Test with another verb
        result = conjugator.future_simple("دید", passive=True)
        expected = ['دیده خواهم شد', 'دیده خواهی شد', 'دیده خواهد شد', 'دیده خواهیم شد', 'دیده خواهید شد', 'دیده خواهند شد']
        assert result == expected

    def test_future_simple_negative_passive(self, conjugator):
        # Test with example from docstring
        result = conjugator.future_simple("شناخت", negative=True, passive=True)
        expected = ['شناخته نخواهم شد', 'شناخته نخواهی شد', 'شناخته نخواهد شد', 'شناخته نخواهیم شد', 'شناخته نخواهید شد', 'شناخته نخواهند شد']
        assert result == expected
        
        # Test with another verb
        result = conjugator.future_simple("خواند", negative=True, passive=True)
        expected = ['خوانده نخواهم شد', 'خوانده نخواهی شد', 'خوانده نخواهد شد', 'خوانده نخواهیم شد', 'خوانده نخواهید شد', 'خوانده نخواهند شد']
        assert result == expected
